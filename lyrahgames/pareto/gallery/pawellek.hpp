...
struct pawellek_problem {
  ...
  static constexpr real box_max(size_t index) { return 5; }
  ...
  void evaluate(const generic::range<real> auto& x,
    ...
    const auto t = x[0] * x[0] + x[1] * x[1] + x[2] * x[2];
    y[2] = real(0.5) * t + sin(t);
  }
};

template <std::floating_point real>
pawellek_problem<real> pawellek{};
